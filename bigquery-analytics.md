...
**Introduction chat analytics (backoffice):** `backoffice_backend` → `IntroductionChatAnalyticsService` exposes **`GET introductions/:id/chat-analytics`**. Queries raw **`mujual.events.profile_events`** (not **`profile_events_grouped`**). Filters **`bid`**, introduction time window (**`DATE(ts)`** + **`ts`** bounds), **`pid IN (initiator, responder)`**, and **`JSON_VALUE(ext, '$.partnerId')`** equal to the **other** party’s profile id (product assumption: one active partner chat per profile). Counts are **`COUNT(*)`**.
