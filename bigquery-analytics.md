...
**Introduction chat analytics (backoffice):** `backoffice_backend` → `IntroductionChatAnalyticsService` exposes **`GET introductions/:id/chat-analytics`**. There is **no `introduction_id`** in telemetry. The handler loads both profiles’ **`user_id`** from Postgres, builds the chat **`conversationId`** as **`sort([userIdA, userIdB]).join('_')`** (same as `getConversationId` in code), and queries raw **`mujual.events.profile_events`** (not `profile_events_grouped`, which rolls up across partners). Rows are filtered by **`bid`**, introduction time window (**`DATE(ts)`** + **`ts`** bounds), **`pid IN (initiator, responder)`**, and **`JSON_VALUE(ext, '$.chatId') = conversationId`**. Events without **`chatId`** in **`ext`** (e.g. some **`messageSent`** rows) fall back to **`JSON_VALUE(ext, '$.partnerId')`** matching the **other** party’s profile id for the sender **`pid`**. Counts are **`COUNT(*)`** (one row per event).
